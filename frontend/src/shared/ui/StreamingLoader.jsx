import { FileText, Search, BookOpenCheck, ListFilter, PenLine, ShieldCheck, Sparkles } from 'lucide-react'
...
const PHASES = [
  ...
  { key: 'retrieving', label: 'A pesquisar fontes', helper: 'A localizar legislação, jurisprudência e documentos relevantes.', Icon: BookOpenCheck },
  { key: 'evaluating', label: 'A avaliar resultados', helper: 'A ordenar as fontes por relevância, autoridade e correspondência jurídica.', Icon: ListFilter },
  ...
  { key: 'verifying', label: 'A verificar fundamentos', helper: 'A confirmar os artigos citados e a consistência da resposta final.', Icon: ShieldCheck },
]
...
export function StreamingLoader({ content = '', phase = 'idle', status = '', elapsedMs = 0 }) {
  const cleanContent = cleanStreamText(content)
  const hasContent = cleanContent.length > 0
  const currentPhase = phase === 'idle' ? (hasContent ? 'composing' : 'classifying') : phase
  ...
  const headline = status || (hasContent ? 'A redigir a resposta fundamentada.' : activePhase.label)
