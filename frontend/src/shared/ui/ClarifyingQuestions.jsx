...
export function ClarifyingQuestions({ request, questions, onSelect, disabled = false }) {
  const fallbackQuestion = questions?.[0] || ''
  const prompt = request?.question || fallbackQuestion
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-2xl border border-white/[0.08] bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.025))] shadow-[0_18px_55px_rgba(0,0,0,0.18)]">
        ...
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            <p className="truncate text-[13px] font-semibold tracking-[-0.01em] text-white/88">
              Só falta confirmar um detalhe
            </p>
            <p className="text-[11px] leading-4 text-white/42">
              Escolha uma resposta ou escreva outra no campo abaixo.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-1.5 p-2">
        <p className="px-3 pb-1 pt-2 text-[14px] font-medium leading-6 text-white/88 sm:text-[13px] sm:leading-5">
          {prompt}
        </p>
        {options.map((option, index) => (
          <button
            ...
            type="button"
            disabled={disabled}
            onClick={() => onSelect?.(option, { question: prompt, options })}
            className="group flex w-full items-start gap-3 rounded-xl border border-transparent px-3 py-2.5 text-left transition-all duration-150 hover:border-white/[0.08] hover:bg-white/[0.045] focus-visible:border-[color:var(--accent)]/45 focus-visible:bg-white/[0.05] focus-visible:outline-none disabled:pointer-events-none disabled:opacity-45"
          >
