...
def _fetch_batch(limit: int, offset: int, source_scope: str | None) -> list[dict]:
...
async def run(batch_size: int, source_scope: str | None, sleep_seconds: float) -> None:
    ...
    started = time.time()
    while True:
        items = _fetch_batch(batch_size, offset, source_scope)
        if not items:
            break
