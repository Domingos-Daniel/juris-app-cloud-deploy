"""Benchmark 10 perguntas complexas - provedor OpenCode"""
    ...
    try:
        ...
        return {
            ...
            "legal_basis": [
                ...
                for b in (resp.legal_basis or [])[:4]
            ],
            "answer_preview": resp.answer[:500] if resp.answer else None,
            "error": None,
        }
...
async def main():
    print("=" * 90)
    print("BENCHMARK OPENCODE - 10 PERGUNTAS COMPLEXAS (LEGISLAÇÃO ANGOLANA)")
    print("=" * 90)
    user = validate_login("admin", "Admin123@")
    results = []
    for q in QUESTIONS:
        print(f"\n[{q['id']}/10] {q['category']}... ", end="", flush=True)
        r = await run_one(q, user["id"])
        results.append(r)
        status = (
            r.get("error")
            or f"{r.get('answer_mode', '?')} conf={r.get('confidence', {}).get('score', '?') if r.get('confidence') else '?'} t={r.get('latency_seconds', 0)}s src={r.get('num_sources', 0)}"
        )
        print(status)
    ...
    avg_conf = sum(scores) / max(len(scores), 1) if scores else 0
    ...
    print(f"  Modos: {modes}")
    print(
        f"  Provider: opencode ({os.environ.get('OPENCODE_MODEL', 'deepseek-v4-flash-free')})"
    )
    ...
    for r in results:
        ...
        if r.get("error"):
            ...
            print(
                f"  Lat: {r.get('latency_seconds')}s | Fontes: {r.get('num_sources')}"
            )
            if r.get("validation_issues"):
                print(f"  Issues: {r['validation_issues']}")
            if r.get("legal_basis"):
                print(
                    f"  Bases legais: {json.dumps(r['legal_basis'], ensure_ascii=False)}"
                )
            if r.get("answer_preview"):
                print(f"  Resposta: {r['answer_preview'][:400]}")

    out = Path("C:/Projectos/TCC/backend/data/processed/benchmark_opencode.json")
